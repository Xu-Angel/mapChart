...
/**
 *
 * @param {*} mainLinesData {nodes:[],edges:[]}
 * @param {*} linesPoints
 * @returns
 */
function generateBranchLines(mainLinesData) {
  const allBranchNodes = []
  const allBranchEdges = []

  // 生成支线
  for (let i = 0; i < Math.floor(1 * 4) + 1; i++) {
    // 选择主干节点
    const mainLineId = Math.floor(1 * Object.keys(linesPoints).length)
    const mainLinePoints = linesPoints[mainLineId]
...
const branchEdges = []
    ...
    for (let j = 0; j < Math.floor(1 * 3) + 1; j++) {
      ...
      const branchNode = {
        ...
        style: {
          fill: '#fff',
          stroke: colors[(mainLineId - 1) % colors.length],
          lineWidth: 3,
          fontSize: 12,
          size: [20, 20],
        },
...
// 主干开始延伸支点
  branchEdges.push({
    ...
    for (let k = 0; k < branchNodeIds.length - 1; k++) {
      branchEdges.push({
        source: branchNodeIds[k],
        target: branchNodeIds[k + 1],
        type: 'polyline',
        style: {
          lineWidth: 6,
          radius: 10,
    ...
    // 连接支线节点回主干节点
    if (Math.random() < 0.5) {
      // 50% chance to form a loop
      branchEdges.push({
        source: branchNodeIds[branchNodeIds.length - 1],
        target: `line-${mainLineId}-node-${endIndex}`,
        type: 'polyline',
        style: {
          lineWidth: 6,
          radius: 10,
    ...
    allBranchEdges.push(...branchEdges)
  }

  // Combine all data
  const allNodes = [...mainLinesData.nodes, ...allBranchNodes]
  const allEdges = [...mainLinesData.edges, ...allBranchEdges]
...
const widthBranchData = generateBranchLines({ nodes, edges })
